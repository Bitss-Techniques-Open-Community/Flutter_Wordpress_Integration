...
import 'package:wordpress_integration_flutter/PageDetails.dart';
import 'API.dart';
...
//                              ),
  ...
  Future<Pages> getAllPost() async {
    try {
      final response = await http.get(API.BASE_URL+API.PAGES);
...
//          getImagePost(extractData[i]["featured_media"].toString());
        ...
        return Pages.fromJson(json.decode(response.body));
      } else {
...
class Pages {
  var pagetInfo;
  Pages({this.pagetInfo});
  factory Pages.fromJson(Object json) {
    return new Pages(pagetInfo: json);
  }
}